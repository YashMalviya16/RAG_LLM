...
Original file is located at
    https://colab.research.google.com/drive/1hytQEvL7o7tvdDBzDlqNd4Mis79dEkq1
...
import os
os.environ["NVIDIA_API_KEY"] = "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"
...
invoke_url = "http://llm_client:9000/v1/chat/completions"  # Make sure this is correct
headers = {
    'accept': 'text/event-stream',
    'content-type': 'application/json',
...
# Utility function to extract streaming content
def get_stream_token(entry: bytes):
    """Utility: Extracts ['choices'][0]['delta']['content'] from the streaming response"""
    if not entry:
        return ""
    entry = entry.decode('utf-8')  # Decode bytes to string
    if entry.startswith('data: '):  # Remove the 'data: ' prefix
        try:
            entry = json.loads(entry[5:])  # Convert JSON string to dictionary
        except ValueError:
            return ""
    return entry.get('choices', [{}])[0].get('delta', {}).get('content', "")

# Process and print the streamed response token by token
for line in response.iter_lines():
    entry = line  # Assign the response line to 'entry'
    print(get_stream_token(entry), end="")  # Extract and print token
